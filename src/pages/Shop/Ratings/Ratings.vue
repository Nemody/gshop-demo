          ...
          <li class="rating-item" v-for="(rating, index) in filterRatings" :key="index">
            ...
            <div class="content">
              ...
              <div class="recommend">
                ...
                <span class="item" v-if="item" v-for="(item, index) in rating.recommend" :key="index">{{item}}</span>
              </div>
...
<script>
  ...
  export default {
    data () {
      return {
        selectType: 0, // 选择显示评论的类型  0-->推荐；1-->吐槽；2-->全部
        onlyContent: true
      }
    },
    mounted () {
    ...
    computed: {
      ...
      filterRatings () {
        ...
        /*
         条件1：
         有两种可能性：seleteType--0/1/2
         rateType--0/1
         a: seleteType为2 ，则显示全部评论，不再判断rateType
         b: seleteType为0或1 ，判断rateType是否等于seleteType
         即： seleteType ===2 || seleteType === rateType
         */
        /*
         条件2：
         有两种可能性：onlyContent---true/false
         a: onlyContent为true ,则只显示text有值的评价
         b: onlyContent为false,则不再判断text是否有值
         即： !onlyContent || text.length > 0
         */
